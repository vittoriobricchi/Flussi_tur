...
# leggi il file Excel
df = pd.read_excel('registro_turisti.xlsx', sheet_name=None)
...
if choice == 'Dashboard':
    ...
    filtered_df = df[df['DATA'] == selected_date.strftime('%Y-%m-%d')]
    ...
    if new_count != current_count:
        # Aggiorna il conteggio nel DataFrame df
        df.loc[(df['DATA'] == pd.to_datetime(selected_date)), selected_nationality] = new_count
    ...
    if filtered_df.drop('DATA', axis=1).eq(0).all().all():
        ...
        # verifica se la colonna 'DATA' esiste e se è di tipo datetime
            if 'DATA' in df.columns and pd.api.types.is_datetime64_any_dtype(df['DATA']):
                ...
                df.to_excel('registro_turisti.xlsx', sheet_name='Dati', index=False)
...
if choice == 'Analisi dati':
    ...
    start_date = pd.to_datetime(st.date_input("Data di inizio", value=pd.to_datetime(df["DATA"].min())))
    end_date = pd.to_datetime(st.date_input("Data di fine", value=pd.to_datetime(df["DATA"].max())))
    ...
    if len(nationalities) > 0:
        ...
        # fonde il DataFrame per prepararlo per il grafico
...
if choice == 'Report mensile':
    ...
    selected_year = st.selectbox('Seleziona l\'anno', df['DATA'].dt.year.unique(), key='selected_year')
    ...
    if st.button("Genera tabella"):
        ...
        month_filter = (df['DATA'].dt.month == selected_month) & (df['DATA'].dt.year == selected_year)

        # crea un filtro per il mese precedente
        previous_month = (selected_month - 1) if (selected_month > 1) else 12
        previous_month_filter = (df['DATA'].dt.month == previous_month) & (df['DATA'].dt.year == selected_year)

        # crea un filtro per lo stesso mese dell'anno precedente
        previous_year_filter = (df['DATA'].dt.month == selected_month) & (df['DATA'].dt.year == (selected_year - 1))

        # crea un filtro per tutti gli anni precedenti al mese selezionato
        all_previous_years_filter = (df['DATA'].dt.month == selected_month) & (df['DATA'].dt.year < selected_year)

        # crea i sotto-dataframe con i dati filtrati
        selected_month_df = df[month_filter]
        previous_month_df = df[previous_month_filter]
        previous_year_df = df[previous_year_filter]
        all_previous_years_df = df[all_previous_years_filter]
        ...
        num_previous_years = selected_year - df['DATA'].dt.year.min()
